...
def init_db():
    ...
    conn.execute("CREATE TABLE IF NOT EXISTS interactions (id INTEGER PRIMARY KEY AUTOINCREMENT, case_id TEXT NOT NULL, timestamp TEXT NOT NULL, distress_score REAL NOT NULL, risk_level TEXT NOT NULL, text_content TEXT, audio_url TEXT, transcription TEXT, safety_signal TEXT)")
    cols={r[1] for r in conn.execute("PRAGMA table_info(interactions)").fetchall()}
    for name,typ in [("text_content","TEXT"),("audio_url","TEXT"),("transcription","TEXT"),("safety_signal","TEXT")]:
        if name not in cols: conn.execute(f"ALTER TABLE interactions ADD COLUMN {name} {typ}")
...
def safety_check(text):
    t=(text or "").lower()
    patterns={
        "Self-harm / suicide concern":[r"\bsuicide\b",r"\bsuicidal\b",r"kill myself",r"end my life",r"want to die",r"wanna die",r"take my own life",r"hurt myself",r"self[- ]?harm"],
        "Threat / immediate safety concern":[r"threat",r"threatened",r"threatening",r"someone will kill me",r"they will kill me",r"going to kill me",r"someone is after me",r"immediate danger"]
    }
...
async def analyze_interaction(case_id:str=Form(...), text:str=Form(""), audio:UploadFile|None=File(None)):
    ...
    text_result=text_probs=audio_result=audio_probs=None; transcription=None; safety_signal=safety_check(text); audio_url=None; input_path=wav_path=None
    if has_text:
    ...
    if has_audio:
        try:
            ext=os.path.splitext(audio.filename or ".audio")[1] or ".audio"; saved_name=f"{uuid.uuid4().hex}{ext}"; saved_path=UPLOAD_DIR/saved_name
            content=await audio.read(); saved_path.write_bytes(content); audio_url=f"/media/{saved_name}"
            input_path=str(saved_path); wav_path=str(UPLOAD_DIR/f"{uuid.uuid4().hex}.wav")
            subprocess.run(["ffmpeg","-y","-i",input_path,"-ac","1","-ar","16000","-c:a","pcm_s16le",wav_path],check=True,stdout=subprocess.PIPE,stderr=subprocess.PIPE)
    ...
    # Safety language is a separate high-priority signal, not an emotion label.
    if safety_signal:
        distress_score=24.0; scoring_method += "+safety_override"; risk_level="Severe"
    else: risk_level=get_risk_level(distress_score)
    timestamp=datetime.now().isoformat(); conn=sqlite3.connect(DB_PATH)
    conn.execute("INSERT INTO interactions (case_id,timestamp,distress_score,risk_level,text_content,audio_url,transcription,safety_signal) VALUES (?,?,?,?,?,?,?,?)",(case_id,timestamp,distress_score,risk_level,text or None,audio_url,transcription,safety_signal)); conn.commit(); conn.close()
